...
<body>
	...
	<form action=".\BookReportServlet" method="post" id="form1">
	    ...
	    <div class="container tab-content" id="nav-tabContent">
			<button id = "insertButton" type="button" class="btn btn-lg btn-primary" data-toggle="modal" data-target="#editModal" name = "insertButton" value="<%=request.getParameter("bookreportBK_ID") %>>">＋新增讀書</button><br>
	    	<br>
			<!-- 閱讀心得 -->
			<div class="tab-pane fade show active" id="nav-read" role="tabpanel" aria-labelledby="nav-read-tab">
				...
				<div class="col-md-4">
					...
					<svg class="bd-placeholder-img card-img-top" width="100%" height="250" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid slice" focusable="false" role="img" aria-label="Placeholder: Thumbnail">
						<image xlink:href="<%=reportData.getBk_Pic() %>" width="100%" height="100%"/>
					</svg>
					<div class="card-body">
						<p><%=reportData.getBk_Name()%><p>
						<p class="card-text">評分：<%=reportData.getBr_score() %></p>
						<input type="hidden" id = "edit_pic" name="<%=reportData.getBr_ID() %>BkPic" value="<%=reportData.getBk_Pic() %>">
						<input type="hidden" id = "edit_BkName" name="<%=reportData.getBr_ID() %>BkName" value="<%=reportData.getBk_Name()%>">
						<input type="hidden" id = "edit_MbID" name="<%=reportData.getBr_ID() %>MbID" value="<%=reportData.getMb_ID() %>">
						<input type="hidden" id = "edit_BkID" name="BkID" value="<%=reportData.getBk_ID() %>">
						...
						<input type="hidden" id = "edit_BkWriter" name="<%=reportData.getBr_ID() %>BkWriter" value="<%=reportData.getBk_Writer() %>">
						<input type="hidden" id = "edit_BkPublish" name="<%=reportData.getBr_ID() %>BkPublish" value="<%=reportData.getBk_Publish() %>">
						...
						<div class="d-flex justify-content-between align-items-center">
							<div class="btn-group">		
								<button type="submit" class="btn btn-sm btn-outline-link" name="viewBookReport" value="<%=reportData.getBr_ID() %>">View</button>
							</div>
			...
			<div class="modal" role="dialog" id="editModal" tabindex="-1">
				<div class="modal-dialog modal-lg">
					...
					<div class="modal-footer">
						...
						<button type="submit" class="btn btn-primary" id ="insertButtons" name="insertButtons" value = "insert1">Save changes</button>
					</div>
					</div>
				</div>
			</div>
